...
Solution MPC::Solve(Eigen::VectorXd x0, Eigen::VectorXd coeffs) {
  ...
  for (auto i = 0; i < N-1 ; i++){
  	...
  	solve.X.push_back(solution.x[x_start+i]);
  	solve.Y.push_back(solution.x[y_start+i]);
  	solve.Delta.push_back(solution.x[delta_start+i]);
  	solve.A.push_back(solution.x[a_start+i]);
  }
